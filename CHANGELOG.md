...
### Added
- **Auto-assign category from channel/author name** — When adding a video, if the channel or author name returned by oEmbed exactly matches an existing category name (case-insensitive), that category is automatically selected. Applies to YouTube (`author_name`) and Vimeo (`author_name`). Falls back to the existing keyword-based title suggestion if no channel match is found.
- **Auto-fetch YouTube metadata on add** — When a video is added and the server has a YouTube API key configured, rich metadata (description, publish date, high-resolution thumbnail) is fetched automatically in the background via `/api/youtube/video`. The "Get Details" button on VideoCard and VideoListItem is removed since metadata now arrives without user action.
- **Logo navigates to library** — Clicking the ClipMark logo/wordmark in the header resets the view to the main library (clears active video, closes shared-with-me panel, shows library).
